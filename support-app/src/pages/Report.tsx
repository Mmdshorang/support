...
const STATUS_LABELS: Record<Ticket["status"], { label: string; badge: string }> = {
	open: {
		label: "باز",
...
const TYPE_LABELS: Record<Ticket["typeSupport"], { label: string; badge: string }> = {
	inPerson: {
		label: "حضوری",
...
export default function TicketReport() {
	...
	const totalOpen = filteredTickets.filter((ticket) => ticket.status === "open").length;

	return (
		<div className="space-y-8">
			<header className="space-y-2">
				<h1 className="text-2xl font-bold text-slate-900 dark:text-white">گزارش تحلیلی تیکت‌ها</h1>
				<p className="text-sm text-slate-500 dark:text-slate-300">
					بازبینی روند درخواست‌ها، مقایسه کانال‌های پشتیبانی و استخراج شاخص‌های کلیدی عملکرد برای جلسات مدیریتی.
				</p>
			</header>
			...
			<section className="grid gap-4 md:grid-cols-3">
				<div className="rounded-3xl border border-slate-200/80 bg-white/80 p-5 shadow-sm dark:border-slate-800/60 dark:bg-slate-900/70">
					<p className="text-xs font-semibold text-slate-500 dark:text-slate-300">تیکت‌های باز</p>
					<p className="mt-3 text-3xl font-black text-amber-500">{totalOpen}</p>
					<p className="mt-2 text-xs text-slate-400 dark:text-slate-400">در بازه انتخاب‌شده</p>
				</div>
				<div className="rounded-3xl border border-emerald-200/70 bg-emerald-50/60 p-5 shadow-sm dark:border-emerald-500/20 dark:bg-emerald-500/15">
					<p className="text-xs font-semibold text-emerald-600 dark:text-emerald-200">میانگین زمان رسیدگی</p>
					<p className="mt-3 text-3xl font-black text-emerald-600 dark:text-emerald-200">۲ ساعت</p>
					<p className="mt-2 text-xs text-emerald-500/80 dark:text-emerald-200/80">۴۰٪ سریع‌تر از هفته قبل</p>
				</div>
				<div className="rounded-3xl border border-indigo-200/60 bg-indigo-50/60 p-5 shadow-sm dark:border-indigo-500/20 dark:bg-indigo-500/15">
					<p className="text-xs font-semibold text-indigo-600 dark:text-indigo-200">کانال غالب</p>
					<p className="mt-3 text-3xl font-black text-indigo-600 dark:text-indigo-100">پرتال وب</p>
					<p className="mt-2 text-xs text-indigo-500/80 dark:text-indigo-200/80">۶۵٪ درخواست‌ها</p>
				</div>
			</section>
			...
			<section className="space-y-5 rounded-3xl border border-slate-200/70 bg-white/80 p-6 shadow-sm dark:border-slate-800/60 dark:bg-slate-900/70">
				<header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
					<div className="space-y-1">
						<h2 className="text-lg font-semibold text-slate-900 dark:text-white">فیلتر نتایج</h2>
						<p className="text-xs text-slate-500 dark:text-slate-300">با ترکیب تاریخ و جستجو، گزارش دقیق‌تری مشاهده کنید.</p>
					</div>
					...
					<table className="min-w-full border-separate border-spacing-y-3 text-sm">
						<thead>
							<tr className="text-right text-xs font-semibold text-slate-500 dark:text-slate-300">
								...
								<th className="bg-slate-100/70 px-3 py-2 dark:bg-slate-800/70">مشکل</th>
								...
								<th className="rounded-l-xl bg-slate-100/70 px-3 py-2 dark:bg-slate-800/70">تاریخ</th>
							</tr>
						</thead>
						<tbody>
							{filteredTickets.length > 0 ? (
								filteredTickets.map((ticket) => (
									...
									>
										<td className="rounded-r-3xl px-3 py-4 text-sm font-semibold text-slate-500 dark:text-slate-300">#{ticket.id}</td>
										<td className="px-3 py-4">
											<div className="space-y-1">
												<p className="text-sm font-semibold text-slate-800 dark:text-white">{ticket.name}</p>
												<p className="text-xs text-slate-400 dark:text-slate-300">ارزش بالا</p>
											</div>
										</td>
										<td className="px-3 py-4">
											<p className="font-mono text-xs text-slate-500 dark:text-slate-300">{ticket.phone}</p>
										</td>
										...
										<td className="px-3 py-4">
											<span className={`inline-flex items-center gap-2 rounded-2xl px-3 py-1 text-xs font-semibold ${TYPE_LABELS[ticket.typeSupport].badge}`}>
												{TYPE_LABELS[ticket.typeSupport].label}
											</span>
										</td>
										<td className="px-3 py-4">
											<span className={`inline-flex items-center gap-2 rounded-2xl px-3 py-1 text-xs font-semibold ${STATUS_LABELS[ticket.status].badge}`}>
												{STATUS_LABELS[ticket.status].label}
											</span>
										</td>
										<td className="rounded-l-3xl px-3 py-4 text-xs font-semibold text-slate-500 dark:text-slate-300">{ticket.date}</td>
									</tr>
								))
							) : (
								<tr>
