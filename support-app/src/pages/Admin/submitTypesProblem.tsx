...
export default function IssuesList() {
  ...
  const stats = useMemo(() => {
    ...
    const totalTickets = issues.reduce(
      (sum, issue) => sum + issue.activeTickets,
      0
    );
    const highDemand = issues.filter(
      (issue) => issue.activeTickets >= 5
    ).length;
  ...
  const saveEditing = async (id: string) => {
    ...
    try {
      ...
      setIssues((prev) =>
        prev.map((issue) =>
          issue.id === id ? { ...issue, type: draftValue.trim() } : issue
        )
      );
      cancelEditing();
    ...
    <div className="space-y-8">
      <header className="space-y-2">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
          دسته‌بندی مشکلات سامانه
        </h1>
        <p className="text-sm text-slate-500 dark:text-slate-300">
          مدیریت متمرکز عناوین مشکلات به تیم پشتیبانی کمک می‌کند درخواست‌ها
          سریع‌تر ارجاع داده شوند و گزارش‌ها ساختار یکسانی داشته باشند.
        </p>
      </header>
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-3xl border border-slate-200/70 bg-white/80 p-5 text-sm shadow-sm dark:border-slate-800/60 dark:bg-slate-900/70">
          ...
          <p className="mt-3 text-3xl font-black text-slate-700 dark:text-white">
            {stats.total}
          </p>
          <p className="mt-1 text-xs text-slate-400 dark:text-slate-400">
            سازماندهی شده برای پشتیبانی
          </p>
        </div>
        <div className="rounded-3xl border border-indigo-200/70 bg-indigo-50/60 p-5 text-sm shadow-sm dark:border-indigo-500/20 dark:bg-indigo-500/15">
          <p className="text-xs font-semibold text-indigo-600 dark:text-indigo-100">
            تیکت‌های فعال
          </p>
          <p className="mt-3 text-3xl font-black text-indigo-600 dark:text-indigo-100">
            {stats.totalTickets}
          </p>
          <p className="mt-1 text-xs text-indigo-500/80 dark:text-indigo-100/80">
            در انتظار بررسی یا اقدام
          </p>
        </div>
        <div className="rounded-3xl border border-rose-200/70 bg-rose-50/60 p-5 text-sm shadow-sm dark:border-rose-500/20 dark:bg-rose-500/15">
          <p className="text-xs font-semibold text-rose-600 dark:text-rose-100">
            اولویت بالا
          </p>
          <p className="mt-3 text-3xl font-black text-rose-600 dark:text-rose-100">
            {stats.highDemand}
          </p>
          <p className="mt-1 text-xs text-rose-500/80 dark:text-rose-100/80">
            نیازمند بازنگری فوری
          </p>
        </div>
      </section>
        ...
        <header className="flex flex-col gap-3 pb-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-300">
              دسته‌های واضح باعث می‌شوند تحلیل گزارش‌ها سریع‌تر انجام شود.
            </p>
          </div>
        </header>
      ...
      <section className="rounded-3xl border border-slate-200/70 bg-white/80 p-6 shadow-sm dark:border-slate-800/60 dark:bg-slate-900/70">
        <header className="flex flex-col gap-2 pb-4">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-300">
            برای ویرایش عنوان روی آیتم کلیک کنید.
          </p>
        </header>
              ...
              {isLoading ? (
                ...
                issues.map((issue) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-4 text-xs font-semibold text-slate-500 dark:text-slate-300">
                      {issue.activeTickets}
                    </td>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        {editingId === issue.id ? (
                          ...
                          <button
                            ...
                            className="rounded-xl bg-indigo-500 px-3 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-600"
                          >
                            <Pencil size={15}/>
                          </button>
                        )}
                        <button
                          ...
                          className="rounded-xl bg-rose-500 px-3 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-rose-600"
                        >
                          <Trash size={15}/>
                        </button>
                      </div>
